...
while (1) % the stop criterion is tested at the end of the loop
   ...
   if size(maxmin,2) < 2
        D=x-imf(end,:)'+mean(imf(end,:));
       Len=size(x,1);
       mape=100/Len*sum(abs(x-D)./x);
       rmse=1/Len*sqrt((x-D)'*(x-D));
       ...
       if(ff==0)
        ...
        plot(x,'b');
        hold on; plot(D,'r');
        title(['EMD model(mape=', num2str(mape),', rmse=',num2str(rmse),')']);
        legend( 'Actual Future Price','EMD Result');
       end
       break;
   end
