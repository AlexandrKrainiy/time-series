function ANN(inputSeries,targetSeries)
...
% View the Network
%view(net)
mape=100/Len*sum(abs(cell2mat(targets)-cell2mat(outputs))./cell2mat(targets));
rmse=1/Len*sqrt((cell2mat(targets)-cell2mat(outputs))*(cell2mat(targets)-cell2mat(outputs))');
figure;
plot(cell2mat(targets),'b');
hold on; plot(cell2mat(outputs),'r');
title(['ANN model(mape=', num2str(mape),', rmse=',num2str(rmse),')']);
