...
for i=1:nPop
    ...
    % Evaluation
    particle(i).Cost=CostFunction(target,particle(i).Position);
...
%% Results
mape=100/50*sum(abs(target(101:150)-predictdata)./target(101:150));
rmse=1/50*sqrt((target(101:150)-predictdata)'*(target(101:150)-predictdata));
figure;
plot(1:150, target(1:150),'b');
hold on;
plot(101:150, predictdata,'r');
title(['PSOSVR model(mape=', num2str(mape),', rmse=',num2str(rmse),')']);  
